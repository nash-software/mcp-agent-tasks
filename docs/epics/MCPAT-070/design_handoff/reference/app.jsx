...
const NAV = [
  ...
  { id: "braindump", label: "Brain dump", icon: () => React.createElement(window.Icon.Brain, { size: 16 }), kbd: "3" },
  { id: "notes", label: "Notes", icon: () => React.createElement(window.Icon.Doc, { size: 16 }), kbd: "4" },
  { id: "advisor", label: "Advisor", icon: () => React.createElement(window.Icon.Wand, { size: 16 }), kbd: "5" },
  { id: "agent", label: "Hermes", icon: () => React.createElement(window.Icon.Robot, { size: 16 }), kbd: "6" },
  { id: "artifacts", label: "Artifacts", icon: () => React.createElement(window.Icon.Files, { size: 16 }), kbd: "7" },
  { id: "roadmap", label: "Roadmap", icon: () => React.createElement(window.Icon.Map, { size: 16 }), kbd: "8" },
  { id: "activity", label: "Activity", icon: () => React.createElement(window.Icon.Activity, { size: 16 }), kbd: "9" },
  { id: "completed", label: "Completed", icon: () => React.createElement(window.Icon.CheckCircle, { size: 16 }), kbd: "0" },
];
const NAV_BY_ID = Object.fromEntries(NAV.map((n) => [n.id, n]));
const NAV_GROUPS = [
  { label: "Workspace", ids: ["today", "board", "braindump", "notes"] },
  { label: "Assistants", ids: ["advisor", "agent"] },
  { label: "Library", ids: ["artifacts", "roadmap", "activity", "completed"] },
];
const CAPTURE_MODES = [
  { id: "infer", label: "Infer", hint: "Auto-detect: task or note", icon: () => React.createElement(window.Icon.Wand, { size: 13 }) },
  { id: "task", label: "Task", hint: "Always create a task", icon: () => React.createElement(window.Icon.CheckCircle, { size: 13 }) },
  { id: "note", label: "Note", hint: "Save as a note", icon: () => React.createElement(window.Icon.Doc, { size: 13 }) },
];
...
function CaptureBar({ onCapture, onCaptureNote, onExpand, registerFocus, mode, onMode }) {
  ...
  useEffect(() => { registerFocus((m) => { if (m) onMode(m); inputRef.current && inputRef.current.focus(); }); }, []);

  const PLACEHOLDER = {
    infer: "Capture anything — I'll sort it into a task or note · \u21e7Enter to expand · #project",
    task: "New task — Enter to add · #project to route it",
    note: "Jot a note — Enter to save · #project",
  };
  ...
  const submit = () => {
    ...
    // infer routes to note when the text reads like a thought, otherwise a task
    let kind = mode;
    if (mode === "infer") kind = /^(note|idea|remember|thought|todo think)[:\-]/i.test(title) ? "note" : "task";
    if (kind === "note") {
      onCaptureNote(title.replace(/^(note|idea|remember|thought)[:\-]\s*/i, ""), project);
      setFlash({ text: "Noted", icon: Icon.Doc });
    } else {
      onCapture(title, project);
      setFlash({ text: mode === "infer" ? "Captured as task" : "Captured", icon: Icon.Check });
    }
    setVal(""); setTimeout(() => setFlash(null), 700);
  };
  ...
  return (
    React.createElement("header", { className: "capture-bar" },
      ...
      React.createElement("div", { className: "capture-mode", role: "tablist", "aria-label": "Capture mode" },
        CAPTURE_MODES.map((mo) =>
          React.createElement("button", {
            key: mo.id, className: "cm-btn " + (mode === mo.id ? "on" : ""),
            title: mo.hint, onClick: () => { onMode(mo.id); inputRef.current && inputRef.current.focus(); },
          }, React.createElement("span", { className: "cm-ico" }, mo.icon()), React.createElement("span", { className: "cm-lbl" }, mo.label))
        )
      ),
      React.createElement("div", { className: "capture-input-wrap", "data-mode": mode },
        React.createElement("span", { className: "lead" }, React.createElement(Icon.Plus, { size: 15 })),
        React.createElement("input", {
          ref: inputRef, className: "capture-input",
          placeholder: PLACEHOLDER[mode] || PLACEHOLDER.infer,
          value: val, onChange: (e) => setVal(e.target.value), onKeyDown: onKey,
        }),
        flash && React.createElement("span", { className: "capture-flash" },
          React.createElement(flash.icon, { size: 14 }), flash.text),
...
function SortMenu({ value, onChange }) {
  ...
  const cur = SORTS.find((s) => s.id === value) || SORTS[0];
  return React.createElement("div", { className: "sort-anchor", ref },
    React.createElement("button", { className: "sort-btn " + (open ? "on" : ""), onClick: () => setOpen((o) => !o) },
      React.createElement("span", { className: "sort-glyph" }, "\u2195"),
      "Sort: ", React.createElement("b", null, cur.label),
      React.createElement(Icon.ChevronDown, { size: 13, style: { color: "var(--muted)" } })
    ),
    open && React.createElement("div", { className: "sort-pop" },
      SORTS.map((s) => React.createElement("button", { key: s.id, className: "sort-opt " + (s.id === value ? "sel" : ""), onClick: () => { onChange(s.id); setOpen(false); } },
        React.createElement("span", null, s.label),
        s.id === value && React.createElement(Icon.Check, { size: 13, style: { color: "var(--accent)" } })))
    )
  );
}
...
function App() {
  ...
  const [captureMode, setCaptureModeState] = useState(() => localStorage.getItem("lifeos-capmode") || "infer");
  const [notes, setNotes] = useState(() => LS.notes.map((n) => ({ ...n })));
  const [sortBy, setSortByState] = useState(() => localStorage.getItem("lifeos-sort") || "priority");
  const setCaptureMode = (m) => { setCaptureModeState(m); localStorage.setItem("lifeos-capmode", m); };
  const setSortBy = (s) => { setSortByState(s); localStorage.setItem("lifeos-sort", s); };
  ...
  const captureNote = (title, project) => {
    const proj = projById[project] || projById.GEN;
    const n = { id: "n-new" + captureSeq++, project, area: proj.area, title, body: "", tags: [], at: "just now", fresh: true };
    setNotes((ns) => [n, ...ns]);
    showToast("Saved to Notes");
  };

  // ── Advisor handlers (suggestions act on real tasks) ──
  const advisorHandlers = {
    openTask: (id) => navigateToTask(id),
    commit: (id) => { const tk = taskById(id); if (tk) commit(tk); },
    hermes: (id) => { const tk = taskById(id); if (tk) scheduleForAgent(tk); },
  };
  ...
  useEffect(() => {
    ...
    const h = (e) => {
      ...
      // number keys -> nav (1-9, 0 = 10th)
      if (!e.metaKey && !e.ctrlKey && /^[0-9]$/.test(e.key)) { const idx = e.key === "0" ? 9 : parseInt(e.key, 10) - 1; const n = NAV[idx]; if (n) { setView(n.id); setPanel(null); } return; }
  ...
  const counts = {
    ...
    advisor: window.buildSuggestions ? window.buildSuggestions(tasks, notes, target).length : 0,
  };
  ...
  return (
    React.createElement("div", { className: "app", "data-focus": focus },
      React.createElement(CaptureBar, { onCapture: captureTask, onCaptureNote: captureNote, onExpand: openBrainDump, mode: captureMode, onMode: setCaptureMode, registerFocus: (fn) => (captureFocusRef.current = fn) }),

      // ── left nav ──
      React.createElement("nav", { className: "nav" },
        NAV_GROUPS.map((grp) =>
          React.createElement("div", { key: grp.label, className: "nav-group" },
            React.createElement("div", { className: "nav-group-label section-label" }, grp.label),
            grp.ids.map((id) => {
              const n = NAV_BY_ID[id];
              return React.createElement("div", {
                key: id, className: "nav-item " + (view === id ? "active" : ""),
                onClick: () => { setView(id); setPanel(null); },
              },
                React.createElement("span", { className: "ico" }, n.icon()),
                React.createElement("span", { className: "nav-label" }, n.label),
                counts[id] != null
                  ? React.createElement("span", { className: "count" }, counts[id])
        ...
        favorites.length > 0 && React.createElement("div", { className: "nav-pinned" },
          React.createElement("div", { className: "nav-group-label section-label" }, "Favourites"),
          favorites.map((pref) => {
        ...
        React.createElement("div", { className: "nav-foot" },
          React.createElement("button", { className: "nav-foot-btn primary", onClick: () => captureFocusRef.current && captureFocusRef.current("task") },
            React.createElement(window.Icon.Plus, { size: 14 }), "New task"),
          React.createElement("button", { className: "nav-foot-btn", onClick: () => setCmdkOpen(true) },
            React.createElement(window.Icon.Search, { size: 13 }), "Search", React.createElement("kbd", { style: { marginLeft: "auto" } }, "\u2318K")),
          React.createElement("div", { className: "nav-density" },
            [["compact", "Compact"], ["balanced", "Cozy"], ["airy", "Spacious"]].map(([v, l]) =>
              React.createElement("button", { key: v, className: "nd-btn " + (t.density === v ? "on" : ""), onClick: () => setTweak("density", v) }, l))
          ),
          React.createElement("div", { className: "nav-status" },
            React.createElement("span", { className: "ns-item", title: config.acr_online ? "ACR online" : "ACR offline" },
              React.createElement("span", { className: "status-dot-sm", style: { width: 6, height: 6, borderRadius: "50%", background: config.acr_online ? "var(--green)" : "var(--muted-2)" } }), "ACR"),
            React.createElement("span", { className: "ns-item", title: config.brain_online ? "Brain online" : "Brain offline" },
              React.createElement("span", { className: "status-dot-sm", style: { width: 6, height: 6, borderRadius: "50%", background: config.brain_online ? "var(--green)" : "var(--muted-2)" } }), "Brain")
          )
        )
      ),
      ...
      React.createElement("main", { className: "main" },
        React.createElement("div", { className: "main-inner" },
          view === "today" && React.createElement(React.Fragment, null,
            ...
            React.createElement("div", { className: "today-toolbar" },
              React.createElement(FilterBar, filterProps),
              React.createElement(SortMenu, { value: sortBy, onChange: setSortBy })
            ),
            React.createElement(TodayView, {
              tasks, selectedId, tweaks: t, target, setTarget, animMap, filter, sortBy,
          ...
          view === "advisor" && React.createElement(AdvisorView, { tasks, notes, target, handlers: advisorHandlers }),
          view === "notes" && React.createElement(NotesView, { notes, onNewNote: () => captureFocusRef.current && captureFocusRef.current("note"), filterProps, filter }),
          view === "completed" && React.createElement(CompletedView, { tasks, onOpen: navigateToTask, filterProps, filter }),
