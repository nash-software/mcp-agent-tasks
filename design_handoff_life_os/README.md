...
- **ACR** (Agent Control Room, `acr.nashsoftware.dev`) = the autonomous **execution machine** for **software work**. It's the right-hand ambient panel. Hermes *decides per job* whether it's software that belongs on ACR, and if so **dispatches** it there.
