...
function CandidateQueue({ candidates, selectedId, onCommit, onOpen, onMenu, animMap }) {
    ...
    React.createElement("div", { className: "list-block" },
      ...
      open && order.filter((a) => byArea[a]).map((area) =>
        React.createElement("div", { key: area },
          ...
          byArea[area].sort((a, b) => PRI_RANK[a.priority] - PRI_RANK[b.priority]).map((t) =>
...
function TodayView({ tasks, selectedId, tweaks, target, setTarget, handlers, animMap, filter }) {
  ...
  return (
    React.createElement("div", { className: "fade-up" },
      ...
      candidates.length > 0 && React.createElement(CandidateQueue, {
        ...
        onOpen: handlers.openPeek, onMenu: handlers.menu, animMap,
      })
    )
  );
}
